...
全 `_trunk` ソース（picoruby/cruby/mruby）を順次 generate → import → esa 投稿。続いて `update:ruby_rdoc` を invoke し、最後に `db_copy_to` の iCloud コピーを実行。rdoc も同じ `ruby_knowledge.db` に同居させる。Store は共有して1回だけ開く。
