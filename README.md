...
| ruby/ruby:rdoc/trunk/{ClassName} | cache.ruby-lang.org tarball → 英語原文のまま格納（和訳は chiebukuro-mcp 経由ホスト LLM agent が on-demand） |
...
└── ruby-rdoc-collector/                # ruby/ruby RDoc 英語原文収集（cache.ruby-lang.org tarball、on-demand 翻訳は agent 側）
```
...
| [bash0C7/ruby-rdoc-collector](https://github.com/bash0C7/ruby-rdoc-collector) | `ruby_rdoc_collector` | ruby/ruby RDoc 英語原文収集（on-demand 翻訳は agent 側）|
...
git clone https://github.com/bash0C7/ruby-rdoc-collector
```
...
`rake` / `rake update:*` / `rake import:*` / `rake esa:*` / `rake db:reembed` / `scripts/update_all.rb` などの**書き込み系タスクは特定の Mac（メイン機）でのみ実行**すること。別 Mac で動かすと以下の不整合を起こす:

- production DB (`db/ruby_knowledge.db`) は git 管理外のローカル資産で、`rake` 末尾で iCloud (`db_copy_to`) に**片方向コピー**される（pull 同期なし）
...
ALLOW_WRITE=1 APP_ENV=production bundle exec rake
```
...
`rake` / `rake esa:*` は esa API へ記事を POST する。トークンは `security` コマンドで Keychain に登録する:
...
デフォルトタスクが全パイプライン。全 `_trunk` ソース（picoruby/cruby/mruby）を順次 generate → import → esa 投稿。続いて `namespace :update` 配下の全 `update:*` タスクを動的発見して順次 invoke（現状は `update:picoruby_docs` / `update:ruby_rdoc` / `update:rurema`）。データソースを増やす時は `task :foo` を `namespace :update` に追加するだけで自動で取り込まれる。最後に `db_copy_to` の iCloud コピーを実行。rdoc / rurema / picoruby_docs も同じ `ruby_knowledge.db` に同居させる。Store は共有して1回だけ開く。テスト実行は `bundle exec rake test`。
...
- **`rake` の再実行は慎重に**: Claude CLI は同じ SINCE/BEFORE でも毎回違う文体・長さの記事を生成する。content_hash が一致せず、DB に重複レコードが積み上がる。失敗後の再実行前に `db:scan_pollution` / `esa:find_duplicates` で既存を確認、必要なら cleanup してから再走らせる。
