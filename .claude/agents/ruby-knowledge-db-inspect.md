...
- **Never** make branch / commit-range / release-line claims about rows in the DB. The authoritative source for that provenance is the `[trunk-changes] source=... branch=... date=... prev=... tip=... commits=...` line emitted by `rake generate:*` stdout, which is not visible at inspection time. If the user asks "what branch is row N from?", answer with the `source` column value verbatim and explain that branch lineage is not stored in the DB — do not reconstruct it via `git log` / `git branch` / cache repo poking. Reading and reporting the `source` column itself is fine; interpreting it as a branch claim is not.
